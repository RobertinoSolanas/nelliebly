...
class PoiControllerTest {
	...
	void getPoi_withoutParameters_shouldReturnBadRequest() throws Exception {
		mockMvc.perform(get("/getPoi")).andExpect(status().isBadRequest());
	}
	...
	void getPoi_withMockFalseAndRealCoordinates_shouldReturnData() throws Exception {
		...
		mockMvc.perform(get("/getPoi?lat=52.5200&lon=13.4050&limit=2&mock=false")) // Berlin
																					// coordinates
			.andExpect(status().isOk())
			.andExpect(jsonPath("$.length()").value(3)); // Should return the simulated
															// OpenStreetMap data
	}
